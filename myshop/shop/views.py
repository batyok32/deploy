...
def product_list(request, category_slug=None, filter_slug=None):
    ...
    most_viewed = list(products.order_by('view'))[:count]
...
def product_search(request):
    ...
    if 'query' in request.GET:
        ...
        if form.is_valid():
            ...
            results1 = search_products.filter(
            		translations__name__icontains = query)
            results2 = search_products.filter(
            		category__translations__name__icontains = query)
            results3 = search_products.filter(
            		translations__description = query)
            results4 = search_products.filter(
            		brand__name__icontains = query)
            print("result1:", results1, "result2:", results2, "result3:", results3, "result4:", results4)
            results1.union(results2, results3, results4)
            print(results1)
            #     search=search_vector,
            #     rank=SearchRank(search_vector, search_query)
            # ).filter(search=search_query).order_by('-rank')
        ...
        #         selected='Latest'
            ...
            # Paginator
            paginator = Paginator(results, 6) # 3 posts in each page
            page = request.GET.get('page')
            try:
                results = paginator.page(page)
            except PageNotAnInteger:
